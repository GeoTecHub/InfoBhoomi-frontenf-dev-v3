...
type QueryParamType = 'text' | 'number' | 'select';
...
const QUERY_CATEGORIES_MAP: Record<string, Omit<QueryCategory, 'key'>> = {
  disaster: {
    label: 'Disaster & Risk',
    icon: 'RSK',
    color: '#e74c3c',
    queries: [
      {
        ...
        params: [
          { key: 'elevation', label: 'Max Elevation', type: 'number', default: 5, unit: 'm' },
          {
            key: 'zone',
            label: 'Zone',
            type: 'select',
      ...
      },
      {
        ...
        params: [
          { key: 'slope', label: 'Min Slope', type: 'number', default: 30, unit: 'deg' },
          {
            key: 'soil',
            label: 'Soil Type',
            type: 'select',
      ...
      },
      {
        ...
        params: [{ key: 'buffer', label: 'Coastal Buffer', type: 'number', default: 100, unit: 'm' }],
        sql: (p) =>
  ...
  land: {
    ...
    queries: [
        ...
        params: [
          {
            ...
            label: 'Land Use',
            type: 'select',
            options: ['Residential', 'Commercial', 'Industrial', 'Agricultural', 'Mixed Use', 'Vacant'],
          },
          { key: 'min_area', label: 'Min Area', type: 'number', default: 0, unit: 'perch' },
        ],
        sql: (p) =>
      ...
      },
      {
        ...
        params: [
          { key: 'min_area', label: 'Min Area', type: 'number', default: 10, unit: 'perch' },
          {
            key: 'zone',
            label: 'Zone',
            type: 'select',
  ...
  utilities: {
    ...
    queries: [
        ...
        params: [
          {
            ...
            label: 'Outfall Point',
            type: 'select',
            options: ['Outfall A - Beira', 'Outfall B - Kelani', 'Outfall C - Canal', 'Outfall D - Coast'],
          },
        ],
        sql: (p) =>
      ...
      },
      {
        ...
        params: [{ key: 'radius', label: 'Service Radius', type: 'number', default: 200, unit: 'm' }],
        sql: (p) =>
    ...
    queries: [
      {
        ...
        params: [
          { key: 'min_arrears', label: 'Min Arrears', type: 'number', default: 10000, unit: 'LKR' },
          {
            key: 'ward',
            label: 'Ward',
            type: 'select',
        ...
        params: [
          {
            ...
            label: 'Facility Type',
            type: 'select',
            options: ['Hospital', 'School', 'Playground', 'Library', 'Police Station', 'Fire Station'],
          },
...
export class QueryBuilderComponent implements OnDestroy {
  readonly categories: QueryCategory[] = Object.entries(QUERY_CATEGORIES_MAP).map(([key, value]) => ({
    key,
    ...value,
  }));

  activeCategoryKey = 'disaster';
  activeQuery: QueryDefinition | null = null;
  params: QueryParams = {};
  isRunning = false;
  results: QueryRunResult | null = null;
  showSQL = false;
  history: QueryHistoryEntry[] = [];

  private runTimer: ReturnType<typeof setTimeout> | null = null;

  constructor(private readonly dialogRef: MatDialogRef<QueryBuilderComponent>) {}

  get activeCategory(): QueryCategory {
    return this.categories.find((item) => item.key === this.activeCategoryKey) || this.categories[0];
  }
  ...
  get generatedSQL(): string {
    ...
    return this.activeQuery.sql(this.params);
  }
  ...
  selectQuery(query: QueryDefinition): void {
    ...
    query.params.forEach((param) => {
      defaults[param.key] =
        param.default !== undefined
          ? param.default
          : (param.options && param.options.length ? param.options[0] : '');
    });
    this.params = defaults;
  }

  updateParam(param: QueryParam, value: string | number): void {
    this.params = {
      ...this.params,
      [param.key]: param.type === 'number' && value !== '' ? Number(value) : String(value),
    };
  }
  ...
  runQuery(): void {
    if (!this.activeQuery || this.isRunning) {
      return;
    }
  ...
  applyQuery(): void {
    ...
    this.dialogRef.close({
      category: this.activeCategory.label,
      queryId: this.activeQuery.id,
      queryName: this.activeQuery.name,
      params: this.params,
